...
class WiggumMCPServer {
    server;
    documentCache;
    searchIndex; // site -> index
    cacheTimeout = 10 * 60 * 1000; // 10 minutes cache
    indexTimeout = 30 * 60 * 1000; // 30 minutes for search index
    embeddingPipeline = null;
    embeddingModel = 'Xenova/all-MiniLM-L6-v2'; // Fast, lightweight model
    ...
    async initializeEmbeddingModel() {
        try {
            ...
            this.embeddingPipeline = await pipeline('feature-extraction', this.embeddingModel, { quantized: true } // Use quantized model for faster inference
            );
    ...
    setupTools() {
        ...
        this.server.registerTool('get_ecosystem_tools', {
        ...
        this.server.registerTool('get_site_info', {
            ...
            const siteInfo = RSTACK_SITES[site];
            if (!siteInfo) {
        ...
        this.server.registerTool('search_docs', {
        ...
        this.server.registerTool('advanced_search', {
        ...
        this.server.registerTool('get_docs', {
        ...
        this.server.registerTool('get_page', {
    ...
    async fetchDocumentation(site, format) {
        const siteInfo = RSTACK_SITES[site];
        if (!siteInfo) {
            throw new Error(`Unknown site: ${site}`);
        }
    ...
    findMarkdownPath(llmsContent, requestedPath) {
    ...
    async advancedSearch(query, site, searchMode, maxResults, includeContext, semanticWeight = 0.5) {
    ...
    async buildOrGetIndex(site, includeEmbeddings = false) {
    ...
    async saveIndexToCache(site, index) {
        try {
    ...
    async buildSearchIndex(site, includeEmbeddings = false) {
        const siteInfo = RSTACK_SITES[site];
        const index = {
            terms: new Map(),
            documents: new Map(),
    ...
    async indexDocument(index, docId, title, content, url, includeEmbeddings = false) {
    ...
    async createChunkedEmbeddings(text, chunkSize = 512, overlap = 128) {
        const chunks = [];
    ...
    async searchWithSemantics(index, query, maxResults, includeContext) {
    ...
    async hybridSearch(index, query, maxResults, includeContext, semanticWeight = 0.5) {
        // Get TF-IDF results
    ...
    async searchWithTFIDF(index, query, maxResults, includeContext) {
        const queryTokens = this.tokenize(query.toLowerCase());
        const scores = new Map();
    ...
    extractHighlights(content, queryTokens) {
    ...
    parseAvailablePages(llmsContent) {
        const lines = llmsContent.split('\n');
        const pages = [];
        let currentSection = '';
        for (const line of lines) {
    ...
    async hybridSearchDocumentation(query, site, maxResults = 10) {
        const sitesToSearch = site === 'all' ? Object.keys(RSTACK_SITES) : [site];
        const allResults = [];
        for (const siteKey of sitesToSearch) {
            try {
    ...
    extractAllMarkdownLinks(llmsContent) {
        const lines = llmsContent.split('\n');
        const links = [];
        for (const line of lines) {
    ...
    async fetchMarkdownContent(baseUrl, path) {
