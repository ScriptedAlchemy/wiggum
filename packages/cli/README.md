...
- package-name dependency linking for local workspace edges (including `npm:` + `workspace:` alias specifiers with npm-compatible package targets, local `file:`/`link:`/`portal:` path references, workspace-wrapped `workspace:file|link|portal:` path references, optional query/hash suffix tolerance on alias/path specifiers, and `bundleDependencies` / `bundledDependencies` arrays)
- inferred local edges from source imports (`import`, dynamic `import()`, `require`, `require.resolve`, `import.meta.resolve`, and export-from specifiers) discovered in `src/`, `test/`, `tests/`, `spec/`, `specs/`, and `__tests__/`
