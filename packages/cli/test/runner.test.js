...
describe('Wiggum runner workspace graph', () => {
  ...
  test('resolveRunnerWorkspace ignores workspace/npm alias targets that are not npm-compatible package names', async () => {
    const root = makeTempWorkspace();
    writeJson(path.join(root, 'wiggum.config.json'), {
      projects: ['packages/*'],
    });
    ...
    writeJson(path.join(root, 'packages/invalid-uppercase/package.json'), {
      name: '@scope/Shared',
      version: '1.0.0',
    });
    writeJson(path.join(root, 'packages/app/package.json'), {
      ...
      version: '1.0.0',
      dependencies: {
        'valid-alias': 'workspace:@scope/shared@*',
        'invalid-workspace-alias': 'workspace:@scope/Shared@*',
      },
      optionalDependencies: {
        'invalid-npm-alias': 'npm:@scope/Shared@1.0.0',
      },
    });
  ...
  test('resolveRunnerWorkspace ignores workspace/npm alias targets with invalid leading punctuation', async () => {
    const root = makeTempWorkspace();
    writeJson(path.join(root, 'wiggum.config.json'), {
      projects: ['packages/*'],
    });
    writeJson(path.join(root, 'packages/shared-valid/package.json'), {
      name: '@scope/shared-valid',
      version: '1.0.0',
    });
    writeJson(path.join(root, 'packages/invalid-unscoped/package.json'), {
      name: '_shared',
      version: '1.0.0',
    });
    writeJson(path.join(root, 'packages/invalid-scoped/package.json'), {
      name: '@scope/-shared',
      version: '1.0.0',
    });
    writeJson(path.join(root, 'packages/app/package.json'), {
      ...
      version: '1.0.0',
      dependencies: {
        'valid-alias': 'npm:@scope/shared-valid@workspace:*',
        'invalid-unscoped-alias': 'workspace:_shared@*',
      },
      optionalDependencies: {
        'invalid-scoped-alias': 'npm:@scope/-shared@1.0.0',
      },
    });
    ...
    const appProject = workspace.projects.find((project) => project.name === '@scope/app');
    expect(appProject).toBeDefined();
    expect(appProject.dependencies).toEqual(['@scope/shared-valid']);
    expect(workspace.graph.edges).toContainEqual({
      from: '@scope/shared-valid',
      to: '@scope/app',
      reason: 'manifest',
    });
    expect(workspace.graph.edges).not.toContainEqual({
  ...
  test('resolveRunnerWorkspace resolves direct path protocols with query/hash suffixes', async () => {
    ...
    writeJson(path.join(root, 'packages/app/package.json'), {
      ...
      version: '1.0.0',
      dependencies: {
        'file-shared': 'file:../shared-file?foo=1#bar',
      },
      devDependencies: {
        'link-shared': 'link:../shared-link?foo=1#bar',
      },
      optionalDependencies: {
        'workspace-shared': 'workspace:../shared-workspace?foo=1#bar',
      },
      peerDependencies: {
        'portal-shared': 'portal:../shared-portal?foo=1#bar',
      },
    });
  ...
  test('resolveRunnerWorkspace supports absolute workspace package.json path specifiers with query/hash suffixes', async () => {
    const root = makeTempWorkspace();
    const sharedDir = path.join(root, 'packages/shared');
    ...
    writeJson(sharedPackageJsonPath, {
      name: '@scope/shared',
      version: '1.0.0',
    });
    writeJson(path.join(root, 'packages/app/package.json'), {
      ...
      version: '1.0.0',
      dependencies: {
        'shared-absolute-workspace': `workspace:${sharedPackageJsonPath}?workspace=true#local`,
      },
    });
  ...
  test('resolveRunnerWorkspace supports absolute file/link/portal package.json path specifiers with query/hash suffixes', async () => {
    const root = makeTempWorkspace();
    const sharedFileDir = path.join(root, 'packages/shared-file');
    const sharedLinkDir = path.join(root, 'packages/shared-link');
    const sharedPortalDir = path.join(root, 'packages/shared-portal');
    const sharedFilePackageJsonPath = path.join(sharedFileDir, 'package.json');
    const sharedLinkPackageJsonPath = path.join(sharedLinkDir, 'package.json');
    const sharedPortalPackageJsonPath = path.join(sharedPortalDir, 'package.json');
    writeJson(path.join(root, 'wiggum.config.json'), {
      projects: ['packages/*'],
    });
    writeJson(sharedFilePackageJsonPath, {
      name: '@scope/shared-file',
      version: '1.0.0',
    });
    writeJson(sharedLinkPackageJsonPath, {
      name: '@scope/shared-link',
      version: '1.0.0',
    });
    writeJson(sharedPortalPackageJsonPath, {
      name: '@scope/shared-portal',
      version: '1.0.0',
    });
    writeJson(path.join(root, 'packages/app/package.json'), {
      ...
      version: '1.0.0',
      dependencies: {
        'shared-absolute-file': `file:${sharedFilePackageJsonPath}?workspace=true#local`,
      },
      devDependencies: {
        'shared-absolute-link': `link:${sharedLinkPackageJsonPath}?workspace=true#local`,
      },
      peerDependencies: {
        'shared-absolute-portal': `portal:${sharedPortalPackageJsonPath}?workspace=true#local`,
      },
    });
  ...
  test('resolveRunnerWorkspace supports workspace-wrapped absolute package.json path specifiers with query/hash suffixes', async () => {
    const root = makeTempWorkspace();
    const sharedFileDir = path.join(root, 'packages/shared-file');
    const sharedLinkDir = path.join(root, 'packages/shared-link');
    const sharedPortalDir = path.join(root, 'packages/shared-portal');
    ...
    writeJson(path.join(root, 'packages/app/package.json'), {
      ...
      version: '1.0.0',
      dependencies: {
        'shared-absolute-workspace-file': `workspace:file:${sharedFilePackageJsonPath}?workspace=true#local`,
      },
      devDependencies: {
        'shared-absolute-workspace-link': `workspace:link:${sharedLinkPackageJsonPath}?workspace=true#local`,
      },
      optionalDependencies: {
        'shared-absolute-workspace-portal': `workspace:portal:${sharedPortalPackageJsonPath}?workspace=true#local`,
      },
    });
