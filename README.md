...
Dependency edges are resolved from local package manifests (including `npm:` / `workspace:` alias specifiers, local `file:`/`link:`/`portal:` path references, workspace-wrapped `workspace:file|link|portal:` path references, and `bundleDependencies` / `bundledDependencies` arrays) and inferred local source specifiers (`import`, dynamic `import()`, `require`, `require.resolve`, `import.meta.resolve`, and export-from forms) discovered from `src/`, `test/`, `tests/`, `spec/`, `specs/`, and `__tests__/` files, then executed in deterministic topological order.
